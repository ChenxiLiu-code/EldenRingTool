...
def main():
    ...
    # This extractor uses the bundled MapForGoblins coordinates for Elden Ring
    # Reforged. A generic mod directory must not accidentally receive hundreds
    # of phantom Reforged markers simply because a mod was selected in the UI.
    goods = params.get("EquipParamGoods")
    goods_ids = {int(row.id) for row in goods.rows} if goods else set()
    out = os.path.join(ROOT, "data", "pieces.json")
    if not set(PIECES).issubset(goods_ids):
        with open(out, "w", encoding="utf-8") as f:
            json.dump({"locales": list(LOCALES), "markers": []}, f, ensure_ascii=False)
        print("pieces: selected mod is not detected as Elden Ring Reforged; wrote an empty corpus")
        print(f"-> {out}")
        return
