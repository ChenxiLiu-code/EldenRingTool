...
# keeps these annotations lazy; the project Setup currently requires Python 3.13.
...
def load(game_dir: str | None = None):
    ...
    if os.name != "nt":
        ...
        if not native:
            raise RuntimeError(
                "native Oodle shim not configured; set ER_LINOODLE to a compatible native Oodle shim library")
        try:
