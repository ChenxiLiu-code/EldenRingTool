...
def find_save(explicit=None):
    ...
    hits = []
    for ext in ("err", "sl2"):
        hits.extend(glob.glob(os.path.join(appdata, "EldenRing", "*", f"ER0000.{ext}")))
    if not hits:
        return None
