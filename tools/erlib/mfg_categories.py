...
# The original MFG lists predate Shadow of the Erdtree. These IDs are
# present in the installed game's GoodsName table (including reinforcement).
SPIRIT_ASH_IDS.update(base + level for base in range(2200000, 2220001, 1000) for level in range(11))
SORCERY_IDS.update({
    2004300, 2004310, 2004320, 2004500, 2004510, 2004700, 2004710,
    2004900, 2004910, 2005000, 2006200, 2006210, 2007410, 2007420,
})
INCANTATION_IDS.update({
    2006300, 2006400, 2006650, 2006660, 2006670, 2006680, 2006690,
    2006700, 2006710, 2006800, 2006900, 2006910, 2006920, 2007000,
    2007010, 2007020, 2007200, 2007210, 2007300, 2007600, 2007700,
    2007710, 2007720, 2007730, 2007740, 2007800, 2007810, 2007820,
})
...
def categorise(iid, name, category):
    ...
    if category == 1:
        if iid in (2009600, 2009610, 2009620):
        ...
        if iid in (8590, 8970, 8971, 8972, 8973, 8974):
            return "whetblades"
        ...
        if "bell bearing" in low:
            # Full-collection exception for the roaming/nomadic merchant family.
            # Their bell-bearing names normally contain ``Merchant``; Kalé is
            # the one base-game exception whose item is simply "Kalé's Bell
            # Bearing".  Do *not* broaden this to every NPC bell bearing:
            # Patches, Corhyn, Moore, Ymir, etc. must remain ordinary
            # bell_bearings so the direct death-lot filter can suppress them.
            is_roaming_merchant = (
                "merchant" in low
                or low in {"kalé's bell bearing", "kale's bell bearing"}
            )
            return "merchant_bell_bearings" if is_roaming_merchant else "bell_bearings"
