...
nothing to draw except a coloured dot. This pulls the real sprites out.
    ...
    python tools/extract_icons.py    -> assets/icons/*.png + data/map-icons.json

Output lands under assets/icons/ for the native QML frontend. Like the map
...
def extract_item_icons(dvd, helper, mod, out):
    catalog_path = os.path.join(ROOT, "data", "catalog.json")
    if not os.path.isfile(catalog_path):
        return
    with open(catalog_path, encoding="utf-8") as stream:
        catalog = json.load(stream)
    wanted = {int(row["itemIconId"]) for row in catalog.get("items", []) if row.get("itemIconId")}
    output = os.path.join(out, "items")
    os.makedirs(output, exist_ok=True)
    saved = set()
    for stem in ("01_common", "01_common_dlc01", "01_common_dlc02"):
        layout_path = f"/menu/hi/{stem}.sblytbnd.dcx"
        texture_path = f"/menu/hi/{stem}.tpf.dcx"
        if not modfiles.has(dvd, mod, layout_path) or not modfiles.has(dvd, mod, texture_path):
            continue
        binder = bnd4.BND4(dcx.decompress(modfiles.read(dvd, mod, layout_path), oodle=helper))
        groups = {}
        for entry in binder.entries:
            name = entry.name.replace(chr(92), "/").split("/")[-1]
            if not name.lower().endswith(".layout") or not name.lower().startswith("sb_icon"):
                continue
            root = ET.fromstring(binder.read(entry).decode("utf-8", "replace"))
            for node in root.iter("SubTexture"):
                sprite = (node.get("name") or "").rsplit(".", 1)[0]
                match = re.fullmatch(r"(?:MENU_)?(?:Icon|ItemIcon|Knowledge)_(\d+)", sprite, re.I)
                if not match or int(match[1]) not in wanted:
                    continue
                rect = tuple(int(node.attrib[k]) for k in ("x", "y", "width", "height"))
                groups.setdefault(name[:-7], []).append((int(match[1]), rect))
        if not groups:
            continue
        data = dcx.decompress(modfiles.read(dvd, mod, texture_path), oodle=helper)
        textures = {t.name: t for t in tpflib.parse(data)}
        for sheet, sprites in groups.items():
            tex = textures.get(sheet)
            if tex is None:
                continue
            img, _, _ = tpflib.texture_image(data, tex)
            for ident, (x, y, w, h) in sprites:
                if w <= 0 or h <= 0 or x < 0 or y < 0 or x+w > img.width or y+h > img.height:
                    continue
                crop = img.crop((x, y, x+w, y+h))
                crop.thumbnail((128, 128), LANCZOS)
                crop.save(os.path.join(output, f"{ident}.png")); crop.close()
                saved.add(ident)
            img.close()
    print(f"item icons: {len(saved)}/{len(wanted)} extracted; unavailable icons use category fallback")
...
def main():
    ...
    try:
        ...
        print(f"warning: item icons unavailable, category fallback retained: {exc}")
