...
检测到后续自动步骤完成时，工具会自动确认前面的必要指引步骤，并显示“后续自动确认”。可选步骤和互斥分支仍需各自的完成依据；推定进度按角色重新计算，不会写成手动勾选。托普斯任务还会读取“托普斯的力场”的持久领取记录，恢复已经完成但中间事件标记消失的进度；仅取得铃珠不会被视为任务完成，因为提前击杀他也能取得铃珠。

本地获取路线说明优先读取 `data/tips-zh.json` 中的中文译文，以原文摘要匹配，重新解析资源或重新获取相同原文不会丢失译文。英文原文和来源信息保留在 `data/tips.json` 中。这些说明来自第三方攻略，并非游戏内置文本，中文主要由机器翻译和游戏术语表生成，部分经过人工校对。原文、译文缓存和个人进度均不随仓库发布。

如需翻译新增的本地说明，可额外安装 `ctranslate2`、`sentencepiece`，并运行 `python tools/localize_tips.py --model <解压后的Argos英文到中文模型目录>`。工具日常运行不需要这些翻译依赖或模型。
